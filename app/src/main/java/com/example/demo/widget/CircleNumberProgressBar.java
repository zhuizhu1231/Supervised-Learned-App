...
public class CircleNumberProgressBar extends ProgressBar {
    ...
    public CircleNumberProgressBar(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.styleCircleNumberProgressBar);
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        text.append(mTextVisibility == VISIBLE ? String.valueOf(getProgress()) : "");
        text.append(      mUnitVisibility == VISIBLE ?  mUnit :  "");
        ...
        //绘制文字
        if (mTextVisibility == VISIBLE|| mUnitVisibility == VISIBLE ) {
            mPaint.setStyle(Paint.Style.FILL);
            mPaint.setColor(mTextColor);
            mPaint.setTextSize(mTextSize);
            mPaint.getTextBounds(text, 0, text.length(), mBound);
            canvas.drawText(text.toString(), mRadius - mBound.width() / 2, baseline, mPaint);
        }
